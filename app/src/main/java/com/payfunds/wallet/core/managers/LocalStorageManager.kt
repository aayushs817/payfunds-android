...
import io.payfunds.core.ILockoutStorage
import io.payfunds.core.IPinSettingsStorage
...
class LocalStorageManager(
    ...
    override var failedAttempts: Int?
        get() {
            return when (val attempts = preferences.getInt(FAILED_ATTEMPTS, 0)) {
                0 -> null
                else -> attempts
            }
        }
    ...
    override var lockoutUptime: Long?
        get() {
            return when (val timestamp = preferences.getLong(LOCKOUT_TIMESTAMP, 0L)) {
                0L -> null
                else -> timestamp
            }
        }
