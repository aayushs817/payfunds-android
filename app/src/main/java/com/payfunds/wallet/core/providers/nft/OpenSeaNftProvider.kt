...
import com.payfunds.wallet.entities.nft.NftContractMetadata
...
class OpenSeaNftProvider(
    ...
    override suspend fun collectionEventsMetadata(
        ...
        val response = service.collectionEvents(
            providerUid,
    ...
    override suspend fun assetsBriefMetadata(
        ...
        for (nftUidsChunk in chunkedNftUids) {
            try {
                val response =
                    service.assets(contractAddresses = nftUidsChunk.map { it.contractAddress },
    ...
    private fun collectionMetadata(
        ...
        return NftCollectionMetadata(
            ...
            inceptionDate = response.primary_asset_contracts?.firstOrNull()?.created_date?.let {
                stringToDate(
                    it
                )
            },
