...
import com.payfunds.wallet.core.providers.nft.INftProvider
...
class NftAssetService(
    ...
    private suspend fun loadAsset() {
        try {
            val (assetMetadata, collectionMetadata) = provider.extendedAssetMetadata(
                nftUid,
    ...
    private fun item(asset: NftAssetMetadata, collection: NftCollectionMetadata): Item {
        return Item(
            ...
            sale = asset.saleInfo?.let { saleInfo ->
                SaleItem(
                    saleInfo.type,
                    saleInfo.listings.map { listing ->
                        SaleListingItem(
                            listing.untilDate,
                            PriceItem(listing.price)
                        )
                    })
            },
            owned = isOwned
    ...
    private fun allCoinUids(item: Item): List<String> {
        val priceItems =
            mutableListOf(item.lastSale, item.average7d, item.average30d, item.collectionFloor)
    ...
    data class Item(
        ...
        val bestOffer: PriceItem?
            get() {
                ...
                val sortedOffers =
                    offers.sortedByDescending { it.priceInFiat?.value ?: BigDecimal.ZERO }
    ...
    data class SaleItem(
        ...
        val bestListing: SaleListingItem?
            get() {
                ...
                val sortedListings =
                    listings.sortedBy { it.price.priceInFiat?.value ?: BigDecimal.ZERO }
