...
import com.payfunds.wallet.modules.manageaccount.ManageAccountFragment
import com.payfunds.wallet.modules.manageaccounts.ManageAccountsModule.AccountViewItem
import com.payfunds.wallet.modules.manageaccounts.ManageAccountsModule.ActionViewItem
...
import com.payfunds.wallet.ui.compose.components.HsRadioButton
...
import com.payfunds.wallet.ui.compose.components.subhead2_lucian
...
fun ManageAccountsScreen(navController: NavController, mode: ManageAccountsModule.Mode) {
    ...
    Column(modifier = Modifier.background(color = ComposeAppTheme.colors.tyler).statusBarsPadding()) {
        AppBar(
            ...
            showSpinner = viewModel.isApiLoading.value
        )
...
private fun AccountsSection(
    ...
    CellUniversalLawrenceSection(items = accounts) { accountViewItem ->
        ...
        var isActiveAccount by remember { mutableStateOf(false) }

        LaunchedEffect(true) {

            val account = App.accountManager.account(accountViewItem.accountId)
            account?.let {
                val item =
                    viewModel.twoFactorAuthCheckWalletAddressResponseModel.value?.data?.find {
                        it.walletAddress.equals(getWalletAddress(account), true)
                    }
                is2FAEnabled = item?.multiFactor?.isEnable ?: false

                activeAccount?.let { mActiveAccount ->
                    isActiveAccount = mActiveAccount.id == accountViewItem.accountId
                }
            }
        }
        ...
        RowUniversal(
            ...
            if (!is2FAEnabled || (isActiveAccount && isMultiFactor != true)) {
