package com.payfunds.wallet.modules.coin.tweets
    ...
    fun tweets(user: TwitterUser): List<Tweet> {
        return data.map { rawTweet ->
            ...
            rawTweet.referencedTweets?.firstOrNull()?.let { tweetReference ->
                includes.referencedTweets.find { tweet -> tweet.id == tweetReference.id }
                        ...
                        includes.users.find { user -> user.id == rawReferencedTweet.authorId }
                            ?.let { referencedTweetAuthor ->
                                ...
                                referencedTweet = when (tweetReference.type) {
                                    "quoted" -> Tweet.ReferencedTweetXxx(
                                        Tweet.ReferenceType.Quoted,
                                        tweet
                                    )

                                    "retweeted" -> Tweet.ReferencedTweetXxx(
                                        Tweet.ReferenceType.Retweeted,
                                        tweet
                                    )

                                    "replied_to" -> Tweet.ReferencedTweetXxx(
