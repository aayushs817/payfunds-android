...
import com.payfunds.wallet.core.adapters.nft.INftAdapter
...
import com.payfunds.wallet.entities.nft.NftAssetShortMetadata
import com.payfunds.wallet.entities.nft.NftCollectionShortMetadata
import com.payfunds.wallet.entities.nft.NftKey
import com.payfunds.wallet.entities.nft.NftRecord
...
class NftHoldingsService(
    ...
    private fun handleXRateUpdate(latestRates: Map<String, CoinPrice?>) {
        ...
        _itemsFlow.tryEmit(items.map {
            it.copy(
                assetItems = updateRates(
                    it.assetItems,
                    latestRates
                )
            )
    ...
    private fun updateRates(
        ...
        asset.price?.let { price ->
            latestRates[price.token.coin.uid]?.let { latestRate ->
                asset.copy(
                    priceInFiat = CurrencyValue(
                        xRateRepository.baseCurrency,
                        price.value.multiply(latestRate.value)
                    ),
    ...
    private fun handle(adaptersMap: Map<NftKey, INftAdapter>) {
        ...
        adaptersMapScope?.launch {
            nftMetadataManager.addressMetadataFlow.filterNotNull()
                .collect { (nftKey, addressMetadata) ->
    ...
    private fun syncNftItemMap() {
        ...
        for ((blockchainType, records) in recordMap) {
            ...
            for (record in records) {
                ...
                val collectionMetadata =
                    collectionMetadataMap[assetMetadata.providerCollectionUid] ?: continue
                ...
                if (collectionItem == null) {
                    ...
                    nftItemMap[uid] =
                        collectionItem.copy(nftItems = collectionItem.nftItems + listOf(nftItem))
                }
            }
        }
    ...
    private fun syncItems() {
        ...
        val coinUids =
            items.map { it.assetItems.mapNotNull { asset -> asset.price?.token?.coin?.uid } }
                .flatten().distinct()
        xRateRepository.setCoinUids(coinUids)

        val latestRates = xRateRepository.getLatestRates()
        val itemsWithCurrencyValues =
            items.map { it.copy(assetItems = updateRates(it.assetItems, latestRates)) }
