...
import com.payfunds.wallet.modules.alert.AlertCreateScreen.tokenSymbol
...
class SendTronViewModel(
    ...
    private val transactionViewModel: SendTransactionViewModel,
    ...
    private suspend fun send() = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val txn = adapter.send(amount, addressState.tronAddress!!, feeState.feeLimit)

            transactionViewModel.sendTransactionDetails(
                contractAddress = adapter.contractAddressHex,
                totalAmount = confirmationData.amount.toString(),
                toAddress = addressState.tronAddress!!.base58,
                fromAddress = adapter.ownAddress,
                symbol = adapter.tokenSymbol,
                txnHash = txn,
            )
