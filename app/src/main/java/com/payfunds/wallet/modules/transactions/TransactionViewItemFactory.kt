...
import com.payfunds.wallet.modules.transactionInfo.ColorName
import com.payfunds.wallet.modules.transactionInfo.ColoredValue
...
class TransactionViewItemFactory(
    ...
    private fun doubleValueIconType(
        ...
        nftMetadata: Map<NftUid, NftAssetBriefMetadata> = mapOf(),
        fallbackPlaceholder: Int = R.drawable.coin_placeholder
    ): TransactionViewItem.Icon {
        ...
        if (primaryValue != null) {
            ...
            frontRectangle = false
            frontUrl = null
            frontPlaceHolder = fallbackPlaceholder
        }
        ...
        if (secondaryValue != null) {
            ...
            backRectangle = false
            backUrl = null
            backPlaceHolder = fallbackPlaceholder
        }
    ...
    private fun tryCreateTronSwapViewItem(
        record: TronContractCallTransactionRecord,
        progress: Float?,
        icon: TransactionViewItem.Icon?,
    ): TransactionViewItem? {
        val result = TronSwapClassifier.classify(record) ?: return null

        return createViewItemFromTronSwapTransaction(
            uid = record.uid,
            timestamp = record.timestamp,
            contractAddress = record.contractAddress,
    ...
    private fun createViewItemFromTronSwapTransaction(
        uid: String,
        timestamp: Long,
        ...
        val primaryValue = valueOut?.let { getColoredValue(it, ColorName.Remus) }
        val secondaryValue = valueIn?.let { getColoredValue(it, ColorName.Lucian) }

        return TransactionViewItem(
            uid = uid,
            progress = progress,
            title = Translator.getString(R.string.Transactions_Swap),
            subtitle = mapped(contractAddress, BlockchainType.Tron),
            primaryValue = primaryValue,
            secondaryValue = secondaryValue,
            showAmount = showAmount,
            date = Date(timestamp * 1000),
            icon = icon ?: doubleValueIconType(
                primaryValue = valueOut,
                secondaryValue = valueIn,
                fallbackPlaceholder = R.drawable.trx_icon
            )
        )
    }
