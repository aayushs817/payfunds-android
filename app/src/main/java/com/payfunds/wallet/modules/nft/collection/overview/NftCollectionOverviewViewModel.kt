...
import com.payfunds.wallet.core.eip20TokenUrl
...
import com.payfunds.wallet.modules.nft.collection.NftCollectionModule.Tab
...
class NftCollectionOverviewViewModel(
    ...
    private fun formatNftPrice(nftPrice: NftPrice) =
        numberFormatter.formatCoinShort(
            nftPrice.value,
            nftPrice.token.coin.code,
            nftPrice.token.decimals
        )
        ...
        overviewViewItem = result.getOrNull()?.let { collection ->
            ...
            NftCollectionOverviewViewItem(
                ...
                floorPrice = collection.floorPrice?.let {
                    PriceViewItem(
                        formatNftPrice(it),
                        formatCurrencyValue(it.value, rate)
                    )
                },
                ...
                oneDaySellersAveragePrice = collection.stats1d?.averagePrice?.let {
                    "~${
                        formatNftPrice(
                            it
                        )
                ...
                royalty = collection.royalty?.let {
                ...
                inceptionDate = collection.inceptionDate?.let {
